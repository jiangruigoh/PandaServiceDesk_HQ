...
from date_functions.Query_Date import current_time, current_date_only, previous_date_only, first_previous_date, last_previous_date, \
    trans_previous_check, backend_period_date_check
...
def validator_main(data, company_guid, outlet_name, outlet_code):
    ...
    for trans_i in range(len(min_trans_date)):
        ...
        try:
            ...
            # GOT ISSUES 
            compared_date = trans_previous_check(min_transPO_date)
            if min_transPO_date < compared_date and \
                trans_tablename == "frontend":
                frontend_pos_trans_message = str(trans_tablename).upper() + " POS data store more than 4 years" + "<br>" \
                    "First Transaction Date: " + min_transPO_date
                frontend_pos_trans_stat = "NO"

            if min_transPO_date < compared_date and \
                trans_tablename == "backend":
                backend_pos_trans_message = str(trans_tablename).upper() + " POS data store more than 4 years" + "<br>" \
                    "First Transaction Date: " + min_transPO_date
            ...
            if min_transPO_date > compared_date and \
                trans_tablename == "frontend":
                frontend_pos_trans_message = str(trans_tablename).upper() + " POS data store less than 4 years" + "<br>" \
                    "First Transaction Date: " + min_transPO_date
                frontend_pos_trans_stat = "OK"
            
            if min_transPO_date > compared_date and \
                trans_tablename == "backend":
                backend_pos_trans_message = str(trans_tablename).upper() + " POS data store less than 4 years" + "<br>" \
                    "First Transaction Date: " + min_transPO_date
                backend_pos_trans_stat = "OK"
        except:
            frontend_pos_trans_stat = "NO"
    ...
    if "doesn't exist" not in daily_backend_period and len(daily_backend_period) != 0:
        ...
        for period_i in range(len(daily_backend_period)):
            ...
            daily_process_status = backend_period_date_check(period_date, period_remark_split)
    ...
    # Backup 
    backup_status = backup_checkpoint(data["backup"])
    # print(backup_status)
    ...
    if "Linux" in  os_fullname: 
        if "OK" in ram_res and \
            ...
            linux_message = "Outlet Name: " + outlet_name + "<br>" \
                ...
                + "Hardisk Partition Info: " + partition_res + "<br>" \
                + "Backup Info: " + backup_msg_1 + "<br>" + "Monthly backup Status: " +  backup_msg_2 + "<br>" \
